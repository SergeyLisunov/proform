...
const FAQ: FaqItem[] = [
  ...
  // W16 Day 82 — anti-positioning objection (matches AntiPositioning section)
  {
    ...
    a: 'Bitrix24 и AmoCRM построены для sales-воронок: лиды, сделки, KPI менеджера. В ProForm роли pre-built под спортивную организацию (Owner / Coach / Doctor / Athlete), защита данных на уровне базы (RLS), плюс встроенные сущности под спорт: тренировочные планы, athlete passes, рекомендации врача, ACWR. Bitrix24 — конструктор для продаж. ProForm — готовая система для спорта.',
  },
  ...
  // W16 Day 82 — beta-data continuity objection («что если бета закроется?»)
  {
    ...
    a: 'Все клубы, подключенные в закрытой бете, остаются на платформе с сохранением всех данных. После launch вы выбираете тариф (или остаётесь на специальных условиях beta-cohort — для первых 20 клубов). Если решите уйти — экспорт данных в CSV/PDF доступен на любом этапе, без vendor lock-in. Если ProForm как продукт закроется (маловероятно, но честно говорю что не невозможно) — за 60 дней до shutdown даём полный экспорт + рекомендации по альтернативам.',
  },
]
