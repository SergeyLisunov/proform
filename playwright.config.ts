...
export default defineConfig({
  ...
  projects: [
    ...
    // Ролевые сценарии: каждый файл сам выбирает нужный storageState.
    {
      ...
      dependencies: ['setup'],
      // 60 с вместо глобальных 30 с. Проверки AI-виджета ждут кнопку до 30 с
      // (она рендерится только после загрузки сессии И ответа capabilities), а
      // до этого тест успевает сходить в capabilities и открыть страницу.
      // В 30-секундный бюджет ожидание физически не помещалось: тест умирал
      // по ОБЩЕМУ таймауту раньше, чем истекало ожидание локатора, — и в
      // отчёте это выглядело как «element(s) not found», то есть указывало на
      // мнимое исчезновение виджета. Бюджет теста обязан быть заметно больше
      // самого долгого ожидания внутри него.
      timeout: 60_000,
    },
