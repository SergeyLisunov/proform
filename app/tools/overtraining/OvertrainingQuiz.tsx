...
export default function OvertrainingQuiz() {
  ...
  return (
    <div>
      {/* Hero */}
      <section className="bg-linear-to-br from-rose-50 via-white to-orange-50 border-b border-border">
        <div className="mx-auto max-w-3xl px-5 py-10">
          <div className="inline-flex items-center gap-1.5 rounded-full border border-rose-200 bg-white px-3 py-1 text-[11px] font-bold uppercase tracking-wider text-rose-600 mb-3">
            <Icon name="ki-pulse" className="text-[11px]" />2 минуты — научный тест
          </div>
          ...
          <div className="rounded-2xl border border-border bg-card shadow-xs">
            ...
            <div className="px-5 pt-4">
              <div className="flex items-center justify-between mb-1.5">
                ...
                <div className="text-[11px] font-semibold text-muted-foreground">{progress}%</div>
              </div>
                    ...
                    <button key={i} onClick={() => pickAnswer(i)}
                      className={`w-full rounded-xl border px-4 py-3 text-left text-sm font-medium transition-all ${
                        active
                          ...
                          : 'border-border bg-card hover:border-rose-200 hover:bg-rose-50/30'
                      }`}>
            ...
            <div className="border-t border-border px-5 py-3 flex items-center justify-between">
              <button onClick={prev} disabled={idx === 0}
                className="rounded-lg border border-border bg-card px-3 py-1.5 text-sm font-semibold disabled:opacity-40 hover:bg-muted">
                ← Назад
              </button>
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border p-5 md:p-6"
              ...
              <div className="flex items-start justify-between flex-wrap gap-3">
                ...
                <div className="text-right">
                  ...
                  <div className="mt-2 h-1.5 w-32 rounded-full bg-card border border-border overflow-hidden">
              ...
              <ul className="space-y-2">
                {[...result!.breakdown].sort((a, b) => b.contribution - a.contribution).slice(0, 3).map(b => (
                  <li key={b.id} className="text-sm text-slate-700">
                    ...
                    <span className="text-xs text-muted-foreground ml-1.5">(+{b.contribution.toFixed(1)} балла)</span>
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="rounded-2xl border border-border bg-card p-5">
              ...
              {!sent && result!.recommendations.length > 2 && (
                <form onSubmit={handleSubmit} className="mt-4 p-4 rounded-xl border border-dashed border-slate-300 bg-muted">
                  <p className="inline-flex items-center gap-1.5 text-xs font-bold text-slate-700 mb-2">
                    <Icon name="ki-lock-2" className="text-xs" />Открыть все {result!.recommendations.length} рекомендаций + PDF-чек-лист
                  </p>
                  <input type="email" required placeholder="you@example.com"
                    value={email} onChange={e => setEmail(e.target.value)}
                    className="w-full rounded-lg border border-border px-3 py-2 text-sm outline-hidden focus:border-rose-400" />
