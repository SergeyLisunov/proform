...
import { DEMO_SESSIONS, DEMO_COMPETITIONS, CYCLE_COLORS, EVENT_COLORS, recoveryColor } from '@/lib/utils/data'
...
function parseLocalDate(dateStr: string): Date { return new Date(dateStr + 'T00:00:00') }
...
function buildHeatmap() {
  const map: Record<string, { strain: number; sessions: number; hasComp: boolean }> = {}
  DEMO_SESSIONS.forEach(s => { map[s.date] = { strain: s.strain, sessions: 1, hasComp: false } })
  DEMO_COMPETITIONS.forEach(c => {
    if (map[c.date]) map[c.date].hasComp = true
    else map[c.date] = { strain: 0, sessions: 0, hasComp: true }
  })
  return map
}
...
interface CycleDrawerProps {
  ...
  userId: string
  onClose: () => void
  onCreated: (cycle: CycleBlock) => void
}

function CycleDrawer({ initialDate, userId, onClose, onCreated }: CycleDrawerProps) {
  ...
  function toggleDay(date: string, dayType: DayType) {
    setDayMap(prev => {
      if (prev[date] === dayType) {
        const next = { ...prev }; delete next[date]; return next
      }
      ...
      <div style={{ position: 'relative', width: '100%', maxWidth: 520, height: '100%', background: 'var(--card)', boxShadow: '-8px 0 40px rgba(0,0,0,0.18)', display: 'flex', flexDirection: 'column', transform: visible ? 'translateX(0)' : 'translateX(100%)', transition: 'transform 0.26s cubic-bezier(0.4,0,0.2,1)' }}>
        ...
        <div style={{ display: 'flex', gap: 0, borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
          {[{ id: 'info', label: '1. Параметры' }, { id: 'days', label: '2. Дни цикла' }].map(s => (
            <button key={s.id} onClick={() => { if (s.id === 'days' && !label.trim()) { setError('Сначала введите название'); return } setError(''); setStep(s.id as any) }}
              style={{ flex: 1, padding: '10px 0', fontSize: 12, fontWeight: 600, borderBottom: step === s.id ? `2px solid ${cfg.text}` : '2px solid transparent', color: step === s.id ? cfg.text : 'var(--muted-foreground)', background: 'transparent', cursor: 'pointer', transition: 'all 0.15s', marginBottom: -1 }}>
              {s.label}
            </button>
          ))}
        </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
                  {(Object.entries(CYCLE_TYPE_CFG) as [CycleType, typeof CYCLE_TYPE_CFG[CycleType]][]).map(([key, c]) => (
                    <button key={key} type="button" onClick={() => setType(key)}
                      style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 4, padding: '12px 14px', borderRadius: 12, border: type === key ? `2px solid ${c.border}` : '1.5px solid var(--border)', background: type === key ? c.bg : 'transparent', cursor: 'pointer', transition: 'all 0.15s' }}>
          ...
          {step === 'days' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <p style={{ fontSize: 12, color: 'var(--muted-foreground)', margin: 0 }}>
                Кликните по дню и выберите тип. Это поможет отслеживать структуру цикла на календаре.
              </p>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 4 }}>
                ...
                {Array.from({ length: (parseLocalDate(startDate).getDay() + 6) % 7 }).map((_, i) => (
                  <div key={`empty-${i}`} />
                ))}
                {cycleDays.map(date => {
                  const dt = dayMap[date]
                  const dcfg = dt ? DAY_TYPE_CFG[dt] : null
                  const d = parseLocalDate(date).getDate()
                  return (
                    <DayCell key={date} date={date} day={d} dayType={dt} cfg={cfg} onSelect={toggleDay} />
                  )
                })}
              </div>
        ...
        <div style={{ padding: '16px 24px', borderTop: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: 10, flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={handleClose} style={{ padding: '11px 14px', borderRadius: 12, border: '1.5px solid var(--border)', background: 'transparent', color: 'var(--muted-foreground)', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
              Отмена
            </button>
          </div>
        </div>
...
// ── Day cell component (чтобы избежать inline complexity) ──────────────────────
function DayCell({ date, day, dayType, cfg, onSelect }: {
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      {open && (
        <div style={{ position: 'absolute', top: '100%', left: 0, zIndex: 100, background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: 6, boxShadow: '0 8px 24px rgba(0,0,0,0.12)', minWidth: 140, marginTop: 4 }}>
          {(Object.entries(DAY_TYPE_CFG) as [DayType, typeof DAY_TYPE_CFG[DayType]][]).map(([key, c]) => (
            <button key={key} onClick={() => { onSelect(date, key); setOpen(false) }}
              style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '7px 10px', borderRadius: 8, border: 'none', background: dayType === key ? c.bg : 'transparent', color: c.color, fontSize: 12, fontWeight: 600, cursor: 'pointer', textAlign: 'left', transition: 'background 0.1s' }}>
...
// ── YEAR VIEW ──────────────────────────────────────────────────────────────────
function YearView({ year, onSelect, cycles }: { year: number; onSelect: (date: string) => void; cycles: CycleBlock[] }) {
  const today = todayISO()
  return (
    <div className="flex flex-col gap-5 pf-enter">
      <div className="flex items-center gap-3 flex-wrap">
        {(['macro','meso','micro'] as CycleType[]).map(t => {
          const c = CYCLE_TYPE_CFG[t]
          return (
            ...
            <div key={month} className="bg-card border border-border rounded-xl p-3">
              ...
              <div className="grid grid-cols-7 gap-[2px] mb-1">
                ...
                {cells.map((day, di) => {
                  ...
                  const inCycle = cycles.find(c => dateStr >= c.start_date && dateStr <= c.end_date)
                  const cc = inCycle ? CYCLE_TYPE_CFG[inCycle.type] : null
                  return (
...
// ── QUARTER VIEW ───────────────────────────────────────────────────────────────
function QuarterView({ year, quarter, onSelect, cycles }: { year: number; quarter: number; onSelect: (date: string) => void; cycles: CycleBlock[] }) {
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pf-enter">
      {months.map(mi => {
        ...
        const monthCycles = cycles.filter(c => {
          const cs = parseLocalDate(c.start_date), ce = parseLocalDate(c.end_date)
                ...
                {cells.map((day, di) => {
                  ...
                  return (
                    <div key={di} onClick={() => onSelect(dateStr)}
                      ...
                      style={{ background: cc ? cc.bg : (h ? strainColor(h.strain) : '#FAFAFA'), borderColor: dateStr === today ? '#F97316' : (cc ? cc.border : 'transparent') }}>
...
function MonthView({ year, month, onSelect, selected, savedEvents, cycles, cycleDaysMap }: {
  ...
  const activeCycles = cycles.filter(c => {
    const cs = parseLocalDate(c.start_date), ce = parseLocalDate(c.end_date)
    return cs <= new Date(year, month+1, 0) && ce >= new Date(year, month, 1)
  })

  const eventsByDate = useMemo(() => {
    const map: Record<string, CalendarEvent[]> = {}
    savedEvents.forEach(ev => { if (!map[ev.event_date]) map[ev.event_date] = []; map[ev.event_date].push(ev) })
    return map
  }, [savedEvents])
          ...
          {activeCycles.map((c, i) => {
            ...
            return (
              <span key={i} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-2xs font-semibold border" style={{ background: cc.bg, color: cc.text, borderColor: cc.border }}>
                <span className="w-1.5 h-1.5 rounded-full" style={{ background: cc.text }} />{c.label}
              </span>
            )
          })}
        </div>
      ...
      {weeks.map((week, wi) => (
        <div key={wi} className="grid grid-cols-7 border-b border-border last:border-b-0">
          {week.map((day, di) => {
            ...
            const cycleDay  = day ? cycleDaysMap[dateStr] : undefined
            const cycleDayCfg = cycleDay ? DAY_TYPE_CFG[cycleDay] : null
            const inCycle   = day ? cycles.find(c => dateStr >= c.start_date && dateStr <= c.end_date) : null
            const cycleCfg  = inCycle ? CYCLE_TYPE_CFG[inCycle.type] : null
            const isToday    = dateStr === today
            const isSelected = dateStr === selected
            return (
              <div key={di} onClick={() => day && onSelect(dateStr)}
                ...
                style={{ borderLeft: cycleCfg ? `3px solid ${cycleCfg.text}` : undefined }}>
                {day && (
                  <>
                    ...
                    {/* Метка типа дня */}
                    {cycleDayCfg && (
                      <div className="mb-0.5 px-1 py-0.5 rounded text-[9px] font-bold flex items-center gap-0.5" style={{ background: cycleDayCfg.bg, color: cycleDayCfg.color }}>
                        <i className={`ki-filled ${cycleDayCfg.icon} text-[8px]`} />{cycleDayCfg.label.slice(0,8)}
                      </div>
                    )}
                    {session && (
                    ...
                    {dayEvents.length > 2 && <div className="text-[10px] text-muted-foreground px-1.5">+{dayEvents.length - 2}</div>}
                  </>
                )}
              </div>
            )
...
// ── WEEK VIEW ──────────────────────────────────────────────────────────────────
function WeekView({ year, month, weekStart, onSelect, selected, savedEvents, cycles, cycleDaysMap }: {
  ...
  return (
    <div className="grid grid-cols-7 gap-2 pf-enter">
      {days.map((d, di) => {
        ...
        const cycleDay  = cycleDaysMap[dateStr]
        const cycleDayCfg = cycleDay ? DAY_TYPE_CFG[cycleDay] : null
        const inCycle   = cycles.find(c => dateStr >= c.start_date && dateStr <= c.end_date)
        const cycleCfg  = inCycle ? CYCLE_TYPE_CFG[inCycle.type] : null
        const isToday    = dateStr === today
        ...
        return (
          <div key={di} onClick={() => onSelect(dateStr)}
            ...
            style={{ borderTop: cycleCfg ? `3px solid ${cycleCfg.text}` : undefined }}>
            ...
            {cycleDayCfg && (
              <div className="mb-1.5 px-1.5 py-1 rounded-lg text-2xs font-semibold flex items-center gap-1" style={{ background: cycleDayCfg.bg, color: cycleDayCfg.color }}>
                <i className={`ki-filled ${cycleDayCfg.icon} text-[10px]`} />{cycleDayCfg.label}
              </div>
            )}
            {inCycle && !cycleDayCfg && (
              <div className="mb-1 px-1.5 py-0.5 rounded text-[9px] font-bold border truncate" style={{ background: cycleCfg!.bg, color: cycleCfg!.text, borderColor: cycleCfg!.border }}>
                {inCycle.label.slice(0, 12)}
              </div>
            )}
            {session && (
            ...
            {!session && !comp && dayEvents.length === 0 && !cycleDayCfg && !inCycle && (
...
function DetailPanel({ dateStr, savedEvents, cycles, cycleDaysMap, onAddEvent, onDeleteEvent, onViewEvent, onViewCycle }: {
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-5 flex flex-col gap-4 h-full">
      ...
      {/* Тип дня из цикла */}
      {cycleDayCfg && (
        <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl border" style={{ background: cycleDayCfg.bg, borderColor: cycleDayCfg.color + '40' }}>
          <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: cycleDayCfg.color + '20' }}>
            <i className={`ki-filled ${cycleDayCfg.icon} text-sm`} style={{ color: cycleDayCfg.color }} />
          </div>
          <div>
            <div className="text-2xs font-bold uppercase tracking-wide" style={{ color: cycleDayCfg.color }}>Тип дня</div>
            <div className="text-sm font-semibold" style={{ color: cycleDayCfg.color }}>{cycleDayCfg.label}</div>
          </div>
        </div>
      )}
...
// ── ADD EVENT DRAWER (unchanged logic) ────────────────────────────────────────
function AddEventDrawer({ initialDate, ownerId, onClose, onCreated, mode = 'create', initialEvent, onUpdated, onDeleted }: {
  initialDate: string; ownerId: string; onClose: () => void; onCreated: (event: CalendarEvent) => void
  mode?: 'create' | 'view' | 'edit'; initialEvent?: CalendarEvent
  onUpdated?: (event: CalendarEvent) => void; onDeleted?: (id: string) => void
}) {
  const [mounted, setMounted]         = useState(false)
  const [visible, setVisible]         = useState(false)
  ...
  const [form, setForm] = useState({
    ...
    title: initialEvent?.title ?? '', notes: initialEvent?.notes ?? '',
    start_time: initialEvent?.start_time ?? '', end_time: initialEvent?.end_time ?? '',
  })
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setSaving(true); setError('')
    const result = await createCalendarEvent({ owner_id: ownerId, event_date: form.event_date, event_type: form.event_type, title: form.title.trim(), notes: form.notes.trim() || null, start_time: form.start_time || null, end_time: form.end_time || null })
    if (!result) { setError('Не удалось сохранить. Проверьте RLS политики.'); setSaving(false); return }
  ...
  async function handleUpdate(e: React.FormEvent) {
    ...
    setSaving(true); setError('')
    const result = await updateCalendarEvent(initialEvent.id, { event_date: form.event_date, event_type: form.event_type, title: form.title.trim(), notes: form.notes.trim() || null, start_time: form.start_time || null, end_time: form.end_time || null })
    if (!result) { setError('Не удалось обновить.'); setSaving(false); return }
    onUpdated?.(result); handleClose()
  }
  ...
  return createPortal(
    <div aria-modal="true" role="dialog" style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', justifyContent: 'flex-end' }}>
      ...
      <div style={{ position: 'relative', width: '100%', maxWidth: 440, height: '100%', backgroundColor: 'var(--card)', boxShadow: '-8px 0 40px rgba(0,0,0,0.18)', display: 'flex', flexDirection: 'column', transform: visible ? 'translateX(0)' : 'translateX(100%)', transition: 'transform 0.26s cubic-bezier(0.4,0,0.2,1)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 24px 16px', borderBottom: '1px solid var(--border)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            {selectedType && <div style={{ width: 36, height: 36, borderRadius: 10, background: selectedType.color + '18', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><i className={`ki-filled ${selectedType.icon}`} style={{ color: selectedType.color, fontSize: 16 }} /></div>}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px 24px' }}>
          {drawerMode === 'view' && initialEvent && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {selectedType && <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 20, background: selectedType.color + '12', border: `1.5px solid ${selectedType.color}30`, alignSelf: 'flex-start' }}><i className={`ki-filled ${selectedType.icon}`} style={{ color: selectedType.color, fontSize: 12 }} /><span style={{ fontSize: 12, fontWeight: 700, color: selectedType.color }}>{selectedType.label}</span></div>}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <FieldView label="Название" value={initialEvent.title} />
                <FieldView label="Дата" value={parseLocalDate(initialEvent.event_date).toLocaleDateString('ru-RU', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })} />
                {(initialEvent.start_time || initialEvent.end_time) && <FieldView label="Время" value={[initialEvent.start_time, initialEvent.end_time].filter(Boolean).join(' – ')} />}
                {initialEvent.notes && <FieldView label="Заметки" value={initialEvent.notes} multiline />}
              </div>
              {confirmDelete ? (
          ...
          {(drawerMode === 'create' || drawerMode === 'edit') && (
            <>
              ...
              <form onSubmit={drawerMode === 'edit' ? handleUpdate : handleCreate} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 10, fontWeight: 700, color: 'var(--muted-foreground)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>Тип события</label>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 6 }}>
                    {EVENT_TYPES.map(t => { const active = form.event_type === t.value; return (<button key={t.value} type="button" onClick={() => setForm(f => ({ ...f, event_type: t.value }))} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 10px', borderRadius: 10, textAlign: 'left', border: active ? `2px solid ${t.color}` : '1.5px solid var(--border)', background: active ? t.color + '12' : 'var(--background)', color: active ? t.color : 'var(--muted-foreground)', fontSize: 11, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s' }}><i className={`ki-filled ${t.icon}`} style={{ fontSize: 12, color: active ? t.color : 'var(--muted-foreground)', flexShrink: 0 }} />{t.label}</button>) })}
                  </div>
                </div>
                <div><label style={{ display: 'block', fontSize: 10, fontWeight: 700, color: 'var(--muted-foreground)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 6 }}>Название <span style={{ color: '#F97316' }}>*</span></label>
                <input ref={titleRef} type="text" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} required placeholder="например, Утренняя пробежка…" style={{ width: '100%', borderRadius: 12, border: '1.5px solid var(--border)', padding: '10px 14px', fontSize: 14, outline: 'none', boxSizing: 'border-box', background: 'var(--background)', color: 'var(--foreground)' }} onFocus={e => (e.target.style.borderColor = '#F97316')} onBlur={e => (e.target.style.borderColor = 'var(--border)')} /></div>
                <div><label style={{ display: 'block', fontSize: 10, fontWeight: 700, color: 'var(--muted-foreground)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 6 }}>Дата</label>
                <input type="date" value={form.event_date} onChange={e => setForm(f => ({ ...f, event_date: e.target.value }))} required style={{ width: '100%', borderRadius: 12, border: '1.5px solid var(--border)', padding: '10px 14px', fontSize: 14, outline: 'none', boxSizing: 'border-box', background: 'var(--background)', color: 'var(--foreground)' }} onFocus={e => (e.target.style.borderColor = '#F97316')} onBlur={e => (e.target.style.borderColor = 'var(--border)')} /></div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  <div><label style={{ display: 'block', fontSize: 10, fontWeight: 700, color: 'var(--muted-foreground)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 6 }}>Начало</label><input type="time" value={form.start_time} onChange={e => setForm(f => ({ ...f, start_time: e.target.value }))} style={{ width: '100%', borderRadius: 12, border: '1.5px solid var(--border)', padding: '10px 14px', fontSize: 14, outline: 'none', boxSizing: 'border-box', background: 'var(--background)', color: 'var(--foreground)' }} onFocus={e => (e.target.style.borderColor = '#F97316')} onBlur={e => (e.target.style.borderColor = 'var(--border)')} /></div>
                  <div><label style={{ display: 'block', fontSize: 10, fontWeight: 700, color: 'var(--muted-foreground)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 6 }}>Конец</label><input type="time" value={form.end_time} onChange={e => setForm(f => ({ ...f, end_time: e.target.value }))} style={{ width: '100%', borderRadius: 12, border: '1.5px solid var(--border)', padding: '10px 14px', fontSize: 14, outline: 'none', boxSizing: 'border-box', background: 'var(--background)', color: 'var(--foreground)' }} onFocus={e => (e.target.style.borderColor = '#F97316')} onBlur={e => (e.target.style.borderColor = 'var(--border)')} /></div>
                </div>
                <div><label style={{ display: 'block', fontSize: 10, fontWeight: 700, color: 'var(--muted-foreground)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 6 }}>Заметки</label><textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} rows={3} placeholder="Заметки…" style={{ width: '100%', borderRadius: 12, border: '1.5px solid var(--border)', padding: '10px 14px', fontSize: 14, outline: 'none', resize: 'none', boxSizing: 'border-box', fontFamily: 'inherit', background: 'var(--background)', color: 'var(--foreground)' }} onFocus={e => (e.target.style.borderColor = '#F97316')} onBlur={e => (e.target.style.borderColor = 'var(--border)')} /></div>
                <div style={{ display: 'flex', gap: 8 }}>
...
function FieldView({ label, value, multiline }: { label: string; value: string; multiline?: boolean }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      ...
      {multiline ? <p style={{ fontSize: 14, color: 'var(--foreground)', margin: 0, whiteSpace: 'pre-wrap', lineHeight: 1.5 }}>{value}</p>
                 : <span style={{ fontSize: 14, color: 'var(--foreground)', fontWeight: 500 }}>{value}</span>}
    </div>
  )
}
...
export default function CalendarPage() {
  ...
  // cycleDaysMap: dateStr → DayType (быстрый доступ для рендера)
  const cycleDaysMap = useMemo(() => {
    const m: Record<string, DayType> = {}
    cycleDays.forEach(cd => { m[cd.day_date] = cd.day_type })
    return m
  }, [cycleDays])

  // Загружаем события + циклы
  useEffect(() => {
    if (!user?.id) return
  ...
  const openEventDrawer = useCallback((event: CalendarEvent, mode: 'view' | 'edit' = 'view') => {
    setEventDrawerEvent(event); setEventDrawerMode(mode)
  }, [])
  ...
  const handleEventCreated = useCallback((event: CalendarEvent) => {
    ...
    if (evDate.getFullYear() !== year || evDate.getMonth() !== month) { setYear(evDate.getFullYear()); setMonth(evDate.getMonth()) }
  ...
  const handleCycleCreated = useCallback((cycle: CycleBlock) => {
  ...
  return (
    <div className="flex flex-col gap-5 pf-enter">
      ...
      {/* Активные циклы — горизонтальная полоса */}
      {cycles.length > 0 && (
      ...
      {eventDrawerEvent && user && (
        <AddEventDrawer initialDate={eventDrawerEvent.event_date} ownerId={user.id} mode={eventDrawerMode} initialEvent={eventDrawerEvent}
          onClose={() => setEventDrawerEvent(null)} onCreated={handleEventCreated}
