...
export default async function AdminDashboard() {
  ...
  return (
    <div className="flex flex-col gap-6 pf-page-enter">
      <div>
        <p className="text-xs text-slate-400 uppercase tracking-widest font-semibold">Администрирование</p>
        <h2 className="pf-num text-3xl text-slate-900 mt-0.5">Обзор платформы</h2>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pf-stagger">
        <StatCard label="Всего пользователей" value={totalUsers ?? 0} icon="ki-people"      iconColor="#2563EB" />
        <StatCard label="Атлетов"             value={athletes ?? 0}   icon="ki-abstract-26"  iconColor="#F97316" />
        <StatCard label="Тренеров"            value={coaches ?? 0}    icon="ki-award"        iconColor="#16A34A" />
        <StatCard label="Тренировок"          value={workouts ?? 0}   icon="ki-graph-up"     iconColor="#7C3AED" />
      </div>

      {/* DB schema map */}
      <div className="card bg-white border border-[#E2E8F0] rounded-2xl p-5">
        <p className="pf-num text-xl text-slate-900 mb-4">Схема базы данных</p>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {[
            { grp:'Идентификация', cols:['users','athletes','trainer_athletes'],                    c:'#2563EB' },
            { grp:'Тренировки',   cols:['workouts','cycle_blocks','competitions'],                  c:'#F97316' },
            { grp:'Биометрика',   cols:['daily_metrics'],                                           c:'#16A34A' },
            { grp:'Соцфункции',   cols:['workout_comments','observation_diary'],                    c:'#7C3AED' },
            { grp:'WHOOP поля',   cols:['hrv · rhr · strain','recovery_score','hr_zone_1–5_min'], c:'#D97706' },
            { grp:'Безопасность', cols:['RLS on all tables','Auth trigger','Role functions'],       c:'#DC2626' },
          ].map(({ grp, cols, c }) => (
      ...
      {/* Recent users table */}
      <div className="card bg-white border border-[#E2E8F0] rounded-2xl p-5">
        <p className="pf-num text-xl text-slate-900 mb-4">Последние пользователи</p>
